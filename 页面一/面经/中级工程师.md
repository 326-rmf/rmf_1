...
##	 每个项目都有package.json	package.json是基于node.js才有的
...
#	模块化总结
-	commonJS 双端	module.exports.xx=exports.xxx	浏览器不认识require	-----要用到browserify
...
-	还有函数类创建buffer实例对象
-	let buf2 = Buffer.alloc(10)--------性能比new buffer更强	在堆中开辟的空间别人没用过	堆是共享的
...
-	程序是在内存里面运行的
##	数据库的分类
-	关系型数据库(RDBS)		MySQL,Oracle,DB2,SQL Server(微软)
-	特点是	关系紧密		都是表
-	excel文件	-------	数据库
-	sheet页签	-------	表
-	列头		-------	字段	唯一标识	不允许修改
...
-	常见的状态码
	...
	-	304	请求资源重定向到缓存里	命中了协商缓存		如果客户端发送了一个带条件的GET 请求且该请求已被允许，而文档的内容（自上次访问以来或者根据请求的条件）并没有改变，则服务器应当返回这个304状态码。简单的表达就是：服务端已经执行了GET，但文件未变化。
	-	401	未授权/请求要求用户的身份认证
	-	404	资源没有找到	
...
##	npm link express	express出现问题的时候	server.js启动不了的时候
	...
	return new Promise((resolve,reject)=>{
		...
		xhr.onreadystatechange = () => {
			...
			if(xhr.status >= 200 && xhr.status < 300){
				resolve(JSON.parse(xhr.responseText))//resolve交出正确数据
			}else{
...
-	消息订阅和发布机制	
	...
	-	缺点是乱像菜市场	没有集中式管理
#	redux
	-	集中式管理组件	是一个独立专门用于做状态管理的js库
	-	可以和react angular vue等项目里面配合使用
	-	清晰目录文件结构
	-	作用是集中式管理react应用里面多个组件**共享**的状态
##	使用redux的情况
-	尽量少用
-	某个组件的状态		需要共享
-	某个状态需要在任何地方都可以拿到
...
##	react-redux	将组件分为两大类	是react的插件
-	npm i react-redux 
-	UI组件
	-	负责UI的呈现	不带业务逻辑
	-	通过props接收数据(一般数据和函数)
...
-	index.js
	...
	-	Provider是凌驾于App上的组件	作用是提供相应所需	Provider是顶级组件
-	redux异步编程		在action_creator.js里面
-	export const createIncermentAsyncAction = (value,delay)=>{
	return (dispatch)=>{setTimeout(()=>{
		dispatch(createIncrementAction(value))
	},delay)}}
-	axios解决跨域问题直接就是在package.json里面配置代理
-	"proxy":http://...		要请求的域名地址
-	代理就是转发请求	拦截当前	去往获取数据的地址	获取完成之后	拦截获取数据的地址	去往当前
-	axios的post请求默认将参数转换为了json进而发送给服务器
##	redux步骤
-	配置store.js
-	
##	纯函数	同样的输入得到相同的输出
-	不得改写输入的参数数据值
-	不产生任何副作用	网络请求	输入输出设备
-	不调用Date.now()等不纯的函数
-	redux里的reducer必须是一个纯函数
-	公司里面的ui组件和容器组件是写在一起的
-	
##	SPA理解是单页面多组件应用
...
-	npx webpack直接运行webpack打包文件
-	webpack.config.js里面的配置**libraryTarget:'umd'**//打包生成的库可以通过commonjs/reqirejs/esm语法引入
-	webpack配置里面的output
	-	library:'aUtils'//向外暴露的对象的名称
	-	libraryTarget:'umd'//umd针对具体哪些规范esm  commonjs	amd也就是requirejs规范
...
-	里面的配置		或者是vue.config.js里面配置devServer		proxy:{}
-	devServer{ 
	proxy:{
	//处理以/api开头路径的请求
        '/api':{
...
-	vuex会越来越臃肿的解决办法就是
	-	拆解模块
	-	1.state	(管理data数据) 
	-	2.mutations(直接更新数据状态)  -->mutations就是突变的意思		--》代表数据直接就改变了
		-	 increment (state) {
				state.count++//更新state里面的状态数
				}	
	-	3.actions(间接更新数据状态)	 -->actions间接更新数据 
	-	4.getters()  
	-	5.mutation-type.js(包含n个mutation函数名常量模块)		拆解成了四个模块	不用把四个模块写在一个js文件里面	这样		就不臃肿了		
...
-	npm i -d webpack-dev-server html-webpack-plugin
	-	webpack自动打包更改的文件	html自动引入更改的打包文件
-	babel
	-	npm i babel-loader @babel/core @babel/preset-env -D
	-	处理es6语法	图片等文件
...
**代码结束**
##	编码常见错误
-	当有时间变化变量的事件	记得时刻更新变量的值
-	还有就是忘记了调用函数的本质	到底是谁调用了谁	怎样执行的机制流程
-	hasOwnProperty('xx')//查找自身是否包含这个元素
###	waimai项目代码
...
-	命令式编程就是所有东西都是自己写
##	package.json里面的一些属性意义
-	"license":"MIT"		代表许可认证	MIT	开源的许可	指令的不同规定了只能用		不能改		等等条件限制		MIT是最宽松的指令	当然还有其他严格控制的许可指令
-	"description":"打包工具函数库的描述字段"
-	"keywords":["atguigu","rmf"]//是搜索的关键字	描述
-	"scripts":{"build":"webpack --mode production"}//配置打包命令npm run build
-	"main":"xxx"//找打包文件的路径	**很重要**
##	自定义工具库	暴露的一些模板	文件夹里面有js模块文件
-	文件目录
	-	function文件夹
		-	call.js
		-	bind.js
		-	...其他的js文件模块
		-	当然暴露的函数	可以全部暴露在一个declares.js文件里面		文件名随便写
		-	那么暴露的时候
			-	export {map,every,some}	from './declares'
-	js文件里面的暴露模板
-	import call from './call'
	export function bind(obj,...args){}
	
-	**入口js	文件 向外暴露工具函数**
	-	export {apply} from './function/call'
	-	...
	-	还有暴露方式
...
-	包含一个src文件夹	
	-	有一个	index.js或者main.js	这里的js文件是整个工具库的入口	作用是向外暴露工具函数
		-	export {call} from '../src/array/call'//引入对应的模块
	-	例如包含string文件夹
		-	文件夹里面包含	call.js
			-	js里面注意**export** function call(obj,..args){}
-	webpack.config.js		配置打包文件
	-	module.export={
		mode://开发版/生产版
		//入口文件
		entry:xxx:path.resolve(__dirname,'src/main.js')
		output:{
			filename://打包的文件名
			path:path.resolve(__dirname,'dist')//打包文件的路径
			library://向外暴露的对象的名称	引用里面的方法的时候就要加上	例如 library:"Utils"	那么用里面的方法的时候加上Utils.call(..)
			libraryTarget:""//针对具体的一些模块语法规范
		}
	}
-	package.json文件里面可以配置
-	version//版本号
-	**"main":"dist/xx.js"**//main是入口的意思	去xx文件里面引入文件	
-	可以设置keywords关键词	
##	将自己写的工具库上传到npm 上
-	**记得验证邮箱		还有就是package.json  就是包名不要太怪**
-	发布同样的工具包的时候	记得在package.json里面**修改版本号**
-	然后再npm publish		
-	也可以在里面配置指令scripts:{"build":"webpack"}//指令npm run build就是直接webpack打包文件
-	webpack打包的时候是基于node.js打包的	那么就要遵循commonJs规范
-	生成打包库文件
	-	npm run build
-	发布到npm仓库
	-	前置就是	npm配置中的中央仓库不可以是淘宝镜像
	-	中央仓库指定了npm的中央仓库
	-	npm config set registry https://registry.npmjs.org/
-	注册中央仓库账号
	-	https://www.npmjs.com/
-	npm addUser		登录npm 仓库	依次指定用户名	密码	邮箱
-	npm publish	将库发布到npm 仓库	库的名称一定要唯一
-	npm unpublish --force	强制删除已发布的库
##	下载并使用自定义的工具包
##	工具函数模块暴露和引入的方式
...
-	export default function call(){}
	-	引入方式是
	-	import {default as call} from ''
##	for in循环主要是用来遍历对象的	遍历数组的时候效率低下	for in循环遍历的时候会找原型链
###	npm info xx		可以查看xx的版本信息
###	样式处理
-	webpack里面的loader执行方式是	
	-	从下往上	从右向左
		-	style-loader	css-loader	这是正确的写入顺序
-	css 
...
-	autoprefixer	和	postcss-loader是搭配使用的
	-	方便css样式编码
####	抽取单独打包  	压缩css文件
-	mini-css-extract-plugin	下载这个插件
	-	webpack里面的这个插件的配置	
...
-	c盘创建文件夹
	-	data文件夹
		-	db(data里面的db文件夹)
-	cmd-->指令mongod	启动数据库		唤醒数据库
-	cmd-->指令mongo		连接数据库(新的cmd	旧的cmd不关闭)	连接数据库
-	ctrl+c关闭数据库
-	bin目录下面添加
	-	mongod.cfg文件
-	管理员身份运行
	...
	-	指定运行服务
	-	sc delete MongoDB		可以删除MongoDB服务
-	cmd-->services.msc-->配置服务
##	mongodb基本指令		mongoose操作在d盘mongoCaozuo
-	mongoose模块化编码
-	数据模板板块
-	studentModle     xxxModel
-	连接数据库模板板块
-	db.js
-	CRUD操作模块
-	db	
...
-	show dbs
	-	查看数据库列表
-	use test
	-	操作test数据库	没有test	就创建	test数据库
-	db.students.insert({name:'kobe'})
	-	在当前数据库的students里面插入文件bson		和json区分开来
-	show.collections
	-	展示collections里面的集合
-	127.0.0.1是保留地址	谁都不能配置
-	增删改查
-	create  update(修改)  delete	renda(查询)\
-	增删改查简称位(CRUD)
-	查询命令
	-	db.xx.find({name:'zs'},{id:1})
	-	
-	插入命令
	-	db.xx.insert({name:'',age:10})
	-	db.xx.insertOne({name:'',age:10})
	-	db.xx.insertMany([{name:'cc',age:10},{name:'ax',age:120}])	
-	删除命令
	-	db.xx.remove({age:20})
-	修改命令	
	-	db.xx.update({name:'ax'},{age:20,name:'sd'})
-	mongoose
	-	在node 平台帮助开发者来连接mongodb的包
	-	简单	方便 安全操作mongoDB
-	mongoose的CRUD相对于原生的mongoDB简单的多
-	Create
-	Read
-	Update
-	Delete
-	odm是对象文档模型	对于非关系型数据库
	-	let obj = {"xx":"","yy":""}
-	orm是对象关系模型	对于关系型数据库
	-	
##	环境变量就是cmd指令运行的依据
-	运行的时候直接去找环境变量有无对应的文件
##	端口号1-----》65535		
-	任何一个应用 都运行在某一个端口上
-	不建议使用1--》199端口号	是预留给系统的	一般都使用的是4位的 4位的也尽量不要用1开头
-	80端口	http超文本服务
-	443端口	https加密的超文本传输服务
##	prototype显式原型指向原型对象	__proto__隐式原型指向原型对象
##	回调函数是异步执行的		但是回调函数是特殊的函数的话就会产生例外	比如for循环..
##	3d模型
-	浏览器3d应用
-	WebGL	浏览器3D绘图技术	是js和openGL ES结合在一起
-	three.js是js编写的WebGL第三方库	提供了3D功能
##	get post的区别
-	get请求只URL编码，post支持多种编码方式
-	get请求的记录会留在历史记录中，post请求不会留在历史记录
-	get只支持ASCII字符，post没有字符类型限制
##	方法返回了属性值		标签里面含有属性值	方法来返回
##	通过 $on(eventName, eventHandler) 侦听一个事件
通过 $once(eventName, eventHandler) 一次性侦听一个事件
通过 $off(eventName, eventHandler) 停止侦听一个事件
##	arc函数是右下左上 0 --> 0.5  -->  1 --> 1.5 
##	一个div里面放一个border-box怪异盒模型来适应盒子的大小	盒子大小可以完全适应
##	&times是叉叉号的标志
##	作为后端js的运行平台	
-	node保留了前端浏览器js常用的数据接口
-	依旧基于作用域和原型链
-	区别在于node将其前端中广泛应用的思想迁移到了服务器端
##	node相较于其他语言的一些特点
-	异步IO
-	读取文件
-	var fs = require('fs')
-	fs.readFile('./path1',function(){
console.log('ok file over')
})
##	事件和回调函数
-	事件的编程方式
-	事件的编程方式具有轻量级	松耦合
-	回调函数也是最好的接收异步调用返回数据的方式
##	单线程
-	不用像多线程编程处处在意状态的同步问题
...
-	大量计算占用CPU导致无法继续调用异步I/O
	-	child_process模块可以启动多进程
##	跨平台
-	node兼容windows和unix平台	得益于node架构层面的改动  在操作系统和node上层模块系统之间构建了一层平台架构 就是libuv	目前libuv称为了许多系统实现跨平台的基础组件
-	node面向网络	且擅长并行I/O 可以有效组织更多的硬件资源
##	获取input框的值
