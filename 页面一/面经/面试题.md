...
-	链表变化的时候的初始化的操作
	...
	-	headList.next = head;//获取实际链表的头部索引
	-	let pre = headList//使用数据的时候	数据的索引是必须存在	那么判断索引就可以判断数据是否存在	
	-	pre.?next-->pre指向链表的下一个元素
-	使用二叉树的时候
	-	插入数值的时候既是		root.val
	-	链表也是这样			nodeList.val
##	二叉搜索树
...
-	注意变量动态变化问题	for循环设置的值		循环条件值动态变化是会出现问题的
	-	    for (var i = l1; i--;) {}//for循环的书写就是	i--为true并且每次用到的数值是不一样的
	-	有点像while的书写方法
...
-	字符串转换为数据展示		Number(str[i]+str[i+1])
-	对象属性长度的获取		
...
-	(\d)\1*-----》以这个数据向后匹配相同的这个数据
##	经典代码
-	    for(let i = 1; i < n; i++){
-	 prev = prev.replace(/(\d)\1*/g, item =>`${item.length}${item[0]}`)}-----》匹配字符串描述作用
-	(\d)\1*--->和第一个数据相同的1个或者更多的数据		正则表达式		RegExp
-	    for (var i = l1; i--;) {}//有点像for循环的书写
-	DFS深度优先算法
-	从头走到底--》找到合适的结果--》没有结果就倒退一格--》重新另外一条道路去寻找
-	let visited = new Set()------->visited.has(headA)------>从headA开始后面数据全部一样
	-	判断链表第一个数据节点一样的时候
-	三目运算符  还有&&运算符可以省略if的操作		简洁代码			方便书写
-	split()返回的是**数组**			join()返回的是**字符串**
	-	s[i]&&arr.push(s[i])------->省略了if判断语句
-	字符串变为字符数组
...
-	js输出	print()
##	set
-	set.add(para)来添加元素		set.has(para)拥有元素的时候	元素是要保持唯一的
-	[]==0--->0==0-->true		{}==0---->NaN==0--->false
-	input不是块级元素
-	iframe可以设置document.domin可以实现跨域
##	es6的写法
-	export	var a = 'michael'
##	继承属性
-	margin不可以被继承
##	堆（Heap）  &&    栈(Stack)
-	堆就是数组实现的二叉树	它没有使用父指针或者子指针	堆根据	堆属性	来排序		堆属性决定了树中节点的位置
###	堆分类
-	最大类和最小类
	-	两者的区别就是节点的排序方式
-	堆仅仅使用一个数组来存储数据		不适用指针
-	二叉搜索快		但是堆搜索慢		因为堆的目的是将最大（最小）节点放在最前面	从而进行快速的插入	删除的操作
-	在堆里	当前层级所有系欸但填满之前	不允许开启下一层级的填充
-	堆的高度是	h=floor(log2(n))	2是底		n是堆节点的个数
-	null不属于typeof的运算结果
##	选择器的优先级
-	!important>内联>id>class=伪类>标签选择器
##	线程同步的问题
-	线程同步就是执行相同的操作来获取同步的结果
##	九宫格
-	<table>
	<tr>
	<td>1</td>
...
-	第一行数据的实现
-	border-spacing实现table里面网格的空格没有存在空隙的效果
-	cursor:pointer//光标指向的时候
-	tr:not(:first-child)//不是第一行数据
##	回溯算法多了一个数组来判断数据是否使用		used[i]=true/false
##	服务器运行端口最好是3000-->8000
##	数据密集型应用通常由很多标准组件
-	数据库 缓存 索引 流处理 批处理
##	svn，ibm(收费),集中式版本控制系统	git 分布式版本控制系统
###	集中式版本控制系统
-	集中式版本控制系统就是必须联网才能工作
-	版本库是集中存放在中央服务器的 
	-	工作的时候就要从中央服务器获得最新的版本	然后开始工作 工作完成之后自己工作推送到中央服务器
###	分布式版本控制系统
-	没有中央服务器 每个人的电脑上都有一个完整的版本库 这样工作的时候就不用联网
-	安全性相较于集中式版本控制系统更高	每个人都有完整的版本库 一个人的坏了不可以用了 可以用别人的 但是集中式版本控制系统的中央服务器坏了 所有人都不能工作了
-	分布式版本控制系统也有一台充当‘中央服务器’的电脑 但是作用仅仅是用来交换大家的修改
-	分布式版本控制系统又是还有就是机器抢你打的分支管理
-	几乎每一种版本控制都已某种形式支持分支 使用分支意味你可以从开发主线上分离开来而不影响主线的工作
-	git branch (branchname)    没有branchname的时候自动列出所有的本地分支
-	git checkout (branchname)
-	git merge----->合并分支
###	cpu密集型(计算密集型)
-	比如计算圆周率 对视频进行高清解码分析 靠的就是cpu的运算能力 
-	一般而言cpu占用率非常高	可能任务本身不太需要I/O设备，也可能是程序多线程实现因此屏蔽了等待I/O时间	 
-	计算密集型任务同时进行的数量等于cpu的核心数量
###	I/O密集型  适合脚本语言开发的多线程
-	程序达到性能极限时	cpu占用率还是很低	可能是任务本身需要大量的I/O操作 pipeline就是管线 等同于流水线 典型的就是gpu渲染管线  指明渲染一个画面要经过多少工序 pipeline做的不好	没有充分利用cpu能力
-	对cpu的消耗很少 大部分时间都是在等待I/O操作完成 比如web应用
##	3&&8-------》结果是8
-	toFixed()函数返回的是字符串类型的数据 
-	str.charAt(index)-->返回的是index下面的字符
	-	str[index].charAt()--->获取下标的字符
-	str.charCodeAt(index)---》返回的是index下面字符的unicode编码
	-	获取下标字符的unicode编码
##	unicode编码
-	\u0400-\u04FF
##	