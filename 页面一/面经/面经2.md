...
-   mixin
    ...
    -   **替换型策略**  props methods inject computed **新的同名参数替换为旧的参数**

    -   **合并型策略**  data 通过 set 方法进行合并和重新赋值

    -   **队列型策略**  钩子函数和 watch 将函数存入数组 正序遍历执行
...
-   webpack 异步加载的代码，会保存在一个全局的 webpackJsonp 中。
    webpack4 官方文档提供了模块按需切割加载，配合 es6 的按需加载 import() 方法，
    import() 动态加载文件的魔法注释
    import(/* webpackChunkName: components */'./js/Home.js').then({default: val} => {})
    可以做到减少首页包体积，加快首页的请求速度，只有其他模块，只有当需要的时候才会加载对应 js。

-   // path 已经自动兼容不同操作系统	指定路径
    path.join(__dirname, '')
...
-   在 Webapck4.x 版本之前，我们都是使用 CommonsChunkPlugin 去做分离
    new webpack.optimize.CommonsChunkPlugin({
        ...
        chunks: 'initial',
        minChunks: 2,
    }), // 文件单独抽离出来打包 被 3 个 入口 chunk 共享的模块
...
-   CDN 的方式引入 js 文件 webpack 需要配置 externals 来注册引入的文件
    因为其余组件使用
    externals: {jquery: "jQuery",}	// 注册 jQuery
    文件使用	const $ = require("jquery")
...
-   arr.slice(-2, -1)       数组的截取数据的方法
    ...
    -   if arr has reference data, is shallow copy
...
-   fill()  将数组中所有元素更改为静态值    
    ...
    -   const arr = Array(m).map(() => Array(n).fill(0))

-   filter  创建一个新数组  新数组的元素满足 filter 函数里面提供的条件

    -   filter(function(element, index, array) {}, thisArg)

    -   thisArg 用来指定函数 function 的 this 值

-   find 返回满足测试功能的第一个元素   没有则返回 undefined

    -   find(function(element, index, array) {}, thisArg)

-   findIndex 返回满足测试条件的第一个元素的索引    没有就返回 -1

    -   findIndex(function(element, index, array) {}, thisArg)

-   flat(depth) 指定嵌套数组结构应该展平的深度级别  默认为 1    返回一个新数组

    -   数组的扁平化    可以使用堆栈    可以使用 reduce

-   flatMap()   返回一个新数组

    -   flatMap(function(item, index, array) {}, thisArg)

    -   数组元素经过 map 函数的操作 再来进行 flat(1) 的操作

-   forEach(function(element, index, array) {}, thisArg)    数组每一个元素进行操作  返回 undefined

    -   数组中未初始化的索引属性不会被调用  在迭代过程中已经访问的元素被删除 (shift()) 后面的元素会跳过

    -   forEach() 停止中断操作只能是抛出错误    不能 continue   break return

    -   forEach()   回调函数是一个同步函数  不等待 Promise

-   Array.from()    静态方法从类数组或者可迭代对象创建一个新的**浅拷贝的实例**

    -   Array.from(arrayLike, function map(element, index) {}, thisArg)

    -   浅拷贝的数组的可以执行一次 map 函数, map 函数是可选执行的   map 函数执行返回 map 执行结果数组

-   arr.includes(value, fromIndex)  数组是否包含一个指定的元素值    返回 boolean

    -   区分大小写字符      fromIndex 负值指定的是倒数的数据值

    -   -0 === +0

    -   fromIndex > len // return false     fromIndex < 0   // 从 0 开始搜索

-   arr.indexOf(value, fromIndex)   返回寻找值的下标索引    

-   arr.lastIndexOf(value, fromIndex)   元素在数组中最后出现的索引  没有则返回 -1

-   Array.isArray(target)   

    -   instanceof 和 isArray   isArray 优于 instanceof 因为 isArray 可以检测 iframes

-   arr.join()      // 数组元素连接称为字符串   默认是 ',' 连接 

    -   一个元素为 undefined 或者 null  那么这个元素会被转换为空字符串

-   arr.keys()  会包含没有对应元素值的元素的索引

-   Number(null)    // 0            Number(undefined)   // NaN

-   arr.map(function (element, index, array) {}, thisArg)       // 返回新的数组

    -   [1,2,3].map(parseInt)       // [1,NaN,NaN]

    -   parseInt(value, index)      // parseInt 传入的参数是错误的  index 作为进制转换的标准是不正确的

    -   arr.map(item => parseInt(item, 10))

-   Array.of(para[,para2[,...[,paraN]]])    创建一个可变参数数量的新数组实例    不考虑参数的数量和类型

    -   Array.of(7)     // [7]      Array.of(1, 2)      // [1,2]

    -   Array(7)        // [,,,,,,]

-   arr.push(para)      // 返回数组的新长度

-   arr.reduce(function (acc, cur, index, arr), initialValue)   // initialValue 和 数组元素总和至少为 1

    -   arr 累加器      cur 当前值  index 当前值的下标  arr 当前数组

    -   initialValue    是累加器的初始值

    -   let len = arr.length    // reduce 回调函数调用的次数取决于是否提供累加器的初始值

        -   若是提供了初始值       那么就是调用 len 次回调函数  

        -   若是没有提供初始值      那么就会调用 len - 1 次回调函数     累加器的初始值是数组的第一个元素

        -   **数组为空并且没有提供初始值**  那么就会报错    TypeError

    -   将结果汇总到单个 acc 返回

-   arr.reduceRight(function (acc, cur, index, array) {}, initialValue)     // 

    -   和 reduce  的区别是从右到左

-   reverse()   方法改变原来的数组  数组元素的位置颠倒

    -   类数组对象  {0: 1, 1: 2, 2: 3, length: 3}       // 类似于数组对象   具有 length 属性

-   delete fruits[0]    // 会将数组的元素更改为 undefined

-   arr.toString()  // 数据元素连接为字符串 元素之间具有 ',' 符号

-   arr.some(function (cur, index, array) {}, thisArg)  // return boolean   at least one sucess --> ok

    -   all failed --> failed   --> false

-   arr.shift()     // return deleted value, if arr is [], return undefined

-   arr.keys()      arr.values()    // 分别返回数组的索引 索引值的迭代器对象

    -   迭代器对象调用 next() 方法返回  迭代器对应的数据

    -   {value: '', done: false}        // done 没有结束返回 false      结束返回 true

-   String

-   str.charAt(index)       // 返回指定索引的字符       

    -   index 范围  [0,str.length-1]    超出范围会返回空字符串

-   str.charCodeAt()        // 返回 0 - 65535 之间的整数    表示给定的索引值的 UTF-16 的代码单元

    -   char 字符       charCode 字符编码

    -   str.charCodeAt(0)       // 0 处的字符编码

-   str.codePointAt()   // 返回一个 Unicode 码点值的非负整数

-   str.concat(str1[,...strN])      // 字符串之间的连接形成新的字符串返回

    -   性能方面推荐使用 += 来替代 concat 函数  连接字符串

-   str.endsWith(searchString[,length]) // 判断当前字符串是否是给定字符串结尾的 返回 boolean

    -   length 指定从 [0,length-1]  的字符串来匹配  是否是给定的字符串结尾 

    -   length 是指定匹配字符串的长度

-   String.fromCharCode()       // 返回**UTF-16代码单元序列**组成的字符串 返回字符串  不是字符串对象

    -   String.fromCharCode(65, 66)     // AB

-   String.fromCodePoint()      // 返回**代码点序列**创建的字符串   

-   str.includes()  // 判断一个字符串是否包含于另外一个字符串

    -   str.includes(searchString[,position])       // position 指定开始的索引位置

-   str.indexOf(searchValue[,fromIndex])    // 字符串中首次从 fromIndex 出现的字符串的位置

-   str.lastIndexOf(searchValue[,fromIndex]) // 字符串中最后一个出现搜索字符串的位置

-   str.localeCompare(str2) // 前者是引用字符串 后者是比较字符串    

    -   前者于后者之前  -1  前者于后者之后 1    相同位置返回 0

    -   不同浏览器之间或者不同浏览器版本的返回值是不同的    没有具体的值

-   str.match(pattern)          // 返回字符串匹配正则表达式的结果数据数组

    -   const regex = /A-Z/g        // 正则字面量

    -   没有匹配的正则返回 null     null 没有 length 属性

-   str.matchAll(pattern)      // 返回一个**包含匹配正则表达式的结果以及分组捕获器**的迭代器

    -   正则表达式必须是全局模式        /g          否则会报错      TypeError

    -   参数不是正则表达式对象  会进行隐式转换  new RegExp(pattern)

    -   const pattern = /t(e)(st(\d?))/g;const str = 'test1test2';

        -   [...str.mathAll(pattern)][0]     // ['test1','e','st1','1']

-   str.normalize() // 会按照某种 unicode 的正规形式将当前字符串正规化  

    -   const str1 = '\u0041\u006d';    str1.normalize("NFC");  // 字符串按照 NFC 的格式正规化

    -   正规化的方式    'NFC'   'NFD'

-   str.padEnd(targetLength[,padString])    // 字符串填充到目标长度     字符串填充的字符 padString

    -   targetLength < str.length   // 返回原来数组 原来数组是不会截断的

    -   填充字符在后面

-   str.padStart(targetLength[,padString])  // 字符串填充到目标长度     填充字符在前面

-   String.raw({raw: 'str'}, para1[,...paraN])      // 函数形式调用传入对象必须包含 raw

    -   String.raw({ raw: 'test' }, 0, 1, 2); // 't0e1s2t'      // 间断插入

    -   String.raw`ok${2+3}`        // 'ok5'     用于模板字符串的形式

    -   String.raw() 是一个模板字符串的标签函数 用来获取模板字符串的原始字符串

    -   String.raw`ok\n`    // 'ok\\n'

-   str.repeat(count)       // 构建一个字符串   字符串包含被连接在一起的指定数量的字符串副本

    -   count 默认是0       那么使用默认值的时候返回的是空字符串    ''

    -   1 返回原本的字符串  count 为小数的时候  会自动向下取整数

-   str.replace(pattern, newStr/function)           不改变原来的字符串  返回新的替换的字符串

    -   pattern 是一个字符串时候    会只替换第一个匹配的位置

    -   替换值可以是一个字符串或者是一个回调函数    函数用来创建新的子字符串

    -   replace 函数的参数  

        -   function (str, p1[,...pn], offset, string)  

        -   str 是匹配达到标准的字符串  p1 到 pn 是第一个到第 n 个括号匹配的字符串

    -   replace 的位置直接将匹配的内容更改为另外一个样式的标签

        -   str.replace(new RegExp(searchValue, 'g'), 
        `<b style="background-color: yellow">${searchValue}</b>`)

-   str.search(pattern)     // 返回字符串第一次匹配的索引   否则返回 -1

-   str.split(pattern[,limit])

    -   字符串匹配结果最多只能有 limit 个结果数据   不能超过    超过规定数量的数据会被截断

    -   pattern 中若是包含圆括号    那么圆括号的匹配结果会出现在结果数据之中

-   str.startsWith(searchString[,position])    返回 boolean    

    -   判断当前字符串是否是以 str2 开头的  返回 boolean 

    -   position 设置匹配开始的位置

-   str.toString()  // 返回指定对象的字符串形式     注意 str 是一个对象 记得 Number(str)

-   str.trimEnd()       str.trimStart()     // 字符串后面 或者 前面的空白字符会被去除

    -   别名    trimRight()     trimLeft()

-   str.valueOf()       // 返回 String 对象的原始值     和 toString 方法类似

-   apply call bind

    -   改变函数指针    内部设计是**函数的赋值**

    -   函数的删除  delete target.fn

-   Object

-   Object.assign()     // 将所有**可以枚举的属性**的值从一个或者多个源对象分配到目标对象

    -   Object.assign(target, ...sources)   // 将返回目标值 返回目标对象的引用

    -   **相同的属性值将会被覆盖**      String Symbol 类型的数据都会被拷贝

    -   继承属性和不可枚举的属性是不可拷贝的

-   Object.create(proto, [propertiesObject])        // 创建一个新的对象 现有的对象 proto 来提供新创建的对
    象的 __proto__

    -   proto 为新创建对象的原型对象    

    -   proto 应该是**原始包装对象或者 null**

    -   Object.create() 实现了类式继承

    -   let o = Object.create(null, {para1: {writable: false,configurable: true, value: 'hello'}})

    -   任何对象都是一个函数对象    function F () {}; F.prototype = proto; return new F()

-   Object.defineProperties()       方法直接在一个对象上定义新的属性或者修改现有的属性 并且返回该对象

    -   Object.defineProperties(obj, props) // obj 是**定义或者修改属性的对象**

    -   configurable writable value enumerable      // **数据描述符**   默认是 false

    -   get set     // **访问器描述符**

-   Object.defineProperty()     相比于 Object.defineProperties() **更加精确添加或者修改对象的属性**

    -   Object.defineProperty(obj, prop, descriptor)

    -   **默认情况**下  使用 Object.defineProperty() **添加的属性是不可修改的**

    -   **数据描述符和访问器描述符都是对象**

    -   **访问器描述符可以拥有的键值  configurable enumerable get set**

    -   Object.defineProperty(obj, "key", {
        value: 'rmf',
        writable: false,
        configurable: false,     // 属性是否可以被删除   其他特性除了 value writable 是否可以被修改
        enumerable: false,       // 定义了对象是否可以在 for in 和 Object.keys() 中枚举
        get () {return para},      
        set (newValue) {para = newValue}
    })

    -   prototype 上面**具有一些共享的数据属性**    // 改变之后 其他对象也会受到相应的影响

        -   可以使用 this 来指定对象的数据获取和赋值        // this 指定了特定的对象

        -   this 的应用避免了数据的相互污染 数据之间的独立性

-   Object.entries(obj)    返回一个可枚举的键值对数组对象   

    -   for (const [key,value] of Object.entries(obj)) {
        console.log(`the ${key} index is the ${value}`)
    }

    -   Object.entries(obj) // 循环对象的属性值的顺序   如果索引是数字  那么必定是从小到大的顺序

    -   **将 Object 转换为 Map**    var map = new Map(Object.entries(obj))   

-   Object.freeze(obj)  // 冻结一个对象 冻结对象的属性是不能修改的  **对象的原型也是不能修改的**

    -   Object.freeze(obj)  // **返回和传入参数相同的对象**

    -   如果 obj 的属性是一个对象   那么**这个对象中的属性是可以修改**的    除非这个对象也是一个冻结对象

        -   **函数 数组 对象 是对象**

-   Object.fromEntries(iterable)    // 将**键值对列表转换为对象**   

    -   实现了迭代协议的可迭代对象  类似于 Array Map

    -   Map --> Object      var obj = Object.fromEntries(map)

    -   array --> Object    var obj = Object.fromEntries(arr)

    -   Object --> Entries  对象到键值对列表    Object.entries(obj)     --> [[key,value],...]

    -   Entries --> Object  键值对列表到对象    Object.fromEntries(map) --> {key: value,...}

-   Object.getOwnPropertyDescriptor(obj, prop)  

    -   返回对象上一个**自有属性**对应的**属性描述符** (自有属性**不是在原型上**而是直接在对象上面的属性)

    -   const descriptor1 = Object.getOwnPropertyDescriptor(obj, 'property1')

    -   属性描述符包含  enumerable configurable writable value get set

    -   函数中参数 obj 不是对象的话 会被强制转换为对象
...
-   Object.getOwnPropertyDescriptors(obj)   获取对象所有自身属性的描述符    
    ...
    -   浅拷贝一个对象
        ...
        -   Object.assign()     
            ...
            -   只拷贝源对象的可枚举的自身对象 拷贝时无法拷贝属性的特性 访问器属性被转换为数据属性

            -   无法拷贝对象的原型

        -   解决 Object.assign() 的问题使用 

            -   Object.create(Object.getPrototypeOf(obj), Object.getOwnPropertyDescriptors(obj))

    -   创建子类

        -   function _father () {}; _father.prototype = {}; function _son () {}

        -   _son.prototype = Object.create(_father.prototype, Object.getOwnPropertyDescriptors({}))

-   Object.getOwnPropertyNames() 返回对象自身属性的属性名 **包括不可枚举的属性**返回属性的字符串数组

    -   **但是不包括 Symbol 作为名称属性**

    -   Object.keys()   获取可枚举的属性

-   Object.getOwnPropertySymbols()  **获得对象的所有 Symbol 属性的数组**

    -   所有对象初始化的时候    不会包含任何 Symbol 除非在对象上赋值了 Symbol 否则只会返回空数组

-   Object.getPrototypeOf(obj)     // 返回指定对象的原型对象

-   obj.hasOwnProperty(propName)     返回对象 obj **自身属性**中是否含有指定 propName 的属性值 

    -   含有指定值 返回 true 否则 返回 false

    -   **js 并没有保护 hasOwnProperty** 若是 obj 对象中存在 hasOwnProperty 的同名方法  

    -   会使用 obj 对象中的同名方法 不会使用 Object.prototype.hasOwnProperty()  方法

-   Object.is(value1, value2) 判断两个值是否为同一值
...
-   Object.isExtensible(obj)   判断一个对象是否是可扩展的       
    ...
    -   **可以将对象标记为不可扩展 不可添加新属性**

-   Object.isFrozen(obj)   判断一个对象是否被冻结

    -   **对象变为不可扩展不代表对象被冻结**    但是对象被冻结代表了对象不可扩展

        -   Object.isForzen(Object.preventExtensions(obj))  // false

        -   但是空对象变为不可扩展  那么就代表对象被冻结 因为没有属性可以配置 

    -   一个密封对象也可以是一个冻结对象    但不是必须的

-   prototypeObj.isPrototypeOf(obj)     一个对象是否存在于另一个对象的原型链上

    -   **prototypeObj 是否存在于 obj 的原型链上**

-   Object.isSealed(obj)       判断对象是否是密封的

    -   密封对象的属性是不可配置并且不可删除    **但不一定是不可写的**

-   Object.keys()   返回一个由给定对象的**自身可枚举属性组成的字符串数组**

-   Object.preventExtensions(obj)   将对象变得不可扩展 只是永远不可添加新的属性

    -   但是属性仍然是可被删除的    一旦将对象变为不可扩展的对象    那么永远不可扩展

    -   不存在一个不可扩展的对象再次变为可扩展的对象

-   obj.propertyIsEnumerable(propName)  对象指定的属性是否是可枚举的    **原型链中的属性除外**

-   Object.seal(obj)   封闭一个对象 阻止添加新属性 并且现有属性标记为不可配置 **可写属性不管理**

    -   不可配置属性代表**属性变得不可删除    数据属性不可被重新定义为访问器属性**

    -   不会影响原型链继承的属性

-   Object.setPrototypeOf(obj, prototype)

    -   设置**一个指定的对象的原型到另外一个对象或者 null**

    -   prototype 不是对象或者 null     那么返回原来的对象

-   obj.valueOf()   返回**对象**的原始值

-   Object.values(obj)  返回对象自身的所有**可枚举属性值的数组**

-   在 '(' '[' '/' '+' '-' '`'开头的语句上加上分号  ';'

-   位运算  & | ~ ^         逻辑运算    && || !

-   闭包返回函数    函数内部引用的外部变量内存是不会清除的  

    -   return function () {return num++}

-  下拉框选择事件 onchange 事件     select.onchange = function () {}

    -   选择的 option 属性  this.value

    -   <selection>
        <option value=""></option>
        </selection>

-   @keyframes 的动画的重复次数     infinity (不限制)

    -   transition: animationName time direction count;

    -   dom.style.animationDuration = value + 's'       // 动画的时间的改变

    -   onchange 事件 普遍

    -   input 框的 type 属性 type = "range"     input 框变为了滑块的形式来进行操作

-   Array.from(document.querySelectorAll('.className'))     // 将 nodelist 转换为数组的形式


-   多个 li 的点击事件  绑定在 ul 上面  方便编码
...
-   oninput onchange onpropertychange

    -   oninput 在输入框**数据变化**就触发  脚本触发无效   

    -   onchange 当输入框**失去焦点**才会触发

    -   onpropertychange 脚本触发有效   (IE 专属)
...
-   字体
    ...
    -   dom.style.cssText = 'background-color: transparent; color: unset; font-weight: unset'
...
-   虚拟节点    VDOM
    ...
    -   props children text tag 

    -   虚拟节点转换为真实节点的过程是 创建 tag -> 创建 text -> 设置 props -> 创建 children

    -   插入文本节点    插入子节点  设置标签属性    vnode.el 的使用

-   父节点包含子节点的操作

    -   node1.contains(node2)        // boolean

    -   node1 = node1.parentNode

-   **void 是 js 的一个函数   接收一个参数并且返回值永远是 undefined**

    -   void 0 比 undefined 减少了 3 个字节

    -   undefined 不是 js 的保留字  可以使用 undefined 作为参数的名称

-   邮箱正则

    -   2405825227@qq.com   // /^[\w\.]+@[\da-z\.-]+\.([a-z]+|[\u4E00-\u9FFF]+)$/
...
-   js 返回对象时候的注意点
    ...
    -   返回的函数不写 function 
        ...
        -   直接写 return { fn() {} }     而不是 return { function fn() {} }

        -   对象中书写函数的形式    fn: function () {}          fn() {}

        -   不支持 function fn() {}

-   **直接设置定时器执行**

    -   let timer = setInterval(() => {console.log(count++)}, 100)

-   返回函数        return function fn() {}

-   疑问点

    -   [...args1,...args2]

-   函数的 length 属性是参数的长度      **fn.length**

-   获取特定进制的数据使用  toString(进制)      // 二进制 八进制 十六进制

-   constructor.prototype.para 上的属性是共享的 所有实例初始都指向同一个属性

    -   构造函数里面的属性是实例独特的

-   pattern.test(str)       // 测试字符串中是否包含 pattern 匹配的字符串 return boolean
...
-   表格行数据的改变使用 **append** 而不是 appendChild

    -   listArr.forEach(item => list.append(item))

    -   querySelector 得到的 dom 再次 querySelector 得到 dom 的子元素

-   querySelector(**selectors API 规范**) getElementById(**dom 规范**)

    -   querySelector **静态集合NodeList**      getElementById **动态集合**
...
-   dom     
    ...
    -   dom.nodeName dom.attributes dom.nodeType dom.childNodes dom.textContent

-   dom.childNodes          dom.children

    -   dom.childNodes      **返回元素的子元素集合 包括 HTML 节点,所有属性,文本**

    -   dom.firstChild **第一个 HTML 节点前面有换行空格** 那么 firstChild 返回就不是想要的元素节点

    -   dom.children    **只返回 HTML 节点**    不返回文本节点
...
-   class 中的函数不需要书写 function 

-   JSON.stringify() 在循环引用的时候出现的问题 

    -   JSON.stringify(obj)     // 报错

    -   使用 cycle.js 来解决循环引用问题    

        -   JSON.decycle(obj)   // obj 可以被使用了 循环引用的问题得到解决

-   1 << n      -->   2**n

    -   想到需要使用位运算

-   3 && 8      错误认为是 boolean 值   应该是 通过的数据类型值 (前面一个或者后面一个) 

    -   只是在 if 条件语句判断的时候转换为了 boolean 值

-   vdom 抽象了原本的渲染过程 实现跨平台能力 不局限于浏览器的 DOM 可以是安卓 IOS 的原生组件
...
##  Reflect     所有方法和属性都是静态的

-   Reflect.defineProperty(obj, propkey, attributes) 相对于 Object.defineProperty

    -   前者的返回值是 boolean      后者的返回值是 对象

    -   Reflect.defineProperty(obj, 'prop', {value: ..,})

-   Reflect.apply(fn, thisArg, argsArr)         // 指定函数和参数的执行结果

    -   Reflect.apply(Math.floor, undefined, [1.75])

-   Reflect.construct(target, argsList[,newTarget])         // 返回的是对象实例

    -   var obj = new Foo(...args); var obj = Reflect.construct(Foo, args)

    -   newTarget 是指的是新创建对象的原型对象的 constructor 属性   只是更改了原型对象里面的构造器

        -   Reflect.construct(target, argsList[,newTarget]) // new.target 会指向 target 或者
            newTarget (newTarget 声明的情况下)

    -   Object.create() // 不使用 new 操作符调用构造函数    构造函数内部 new.target 指向 undefined

-   Reflect.deleteProperty(obj, propKey)        // 返回的是 boolean

    -   Reflect.deleteProperty(obj, 'foo')

-   Reflect.get(target, propKey[,receiver])     // receiver 是指定的 getter 的 this 值 可选的

    -   Reflect.get(obj, 'foo')

-   Reflect.getOwnPropertyDescriptor(target, propKey)   // 返回对象的属性描述符

    -   Object.getOwnPropertyDescriptor // 处理非对象数据   会将数据强制转换为对象数据

    -   Reflect.getOwnPropertyDescriptor  // 处理非对象数据   会抛出 TypeError

    -   Reflect.getOwnPropertyDescriptor(obj, 'foo')    // {value:,enumerable:,}

-   Reflect.getPrototypeOf()        // 返回的是指定对象的原型

    -   Reflect.getPrototypeOf(Object.prototype) // null

-   Reflect.has(obj, propKey)   和 in 操作符是一样的        // 返回的是 boolean 

    -   Reflect.has(obj, 'foo');    'foo' in obj // true     

-   Reflect.isExtensible(obj)      // 判断一个对象是否是可扩展的

-   Reflect.ownKeys(obj)        // 返回一个目标对象自身属性值键组成的数组 包括 Symbol

-   Reflect.preventExtensions(obj)      // 阻止新属性添加到对象     返回的是 boolean

-   Reflect.set(obj, propKey, value[,receiver])     // 返回 boolean 表明属性是否设置成功

    -   若是遇到 setter, 那么 receiver 是 setter 里面的 this 指针

    -   Reflect.set(obj, 'prop', 'value')

-   Reflect.setPrototypeOf()        // 返回的是是否操作成功的 boolean 值

    -   Reflect.setPrototypeOf(obj, prototype);  // Reflect.setPrototypeOf({}, null)

##  Proxy 和 Reflect 的联合操作

-   **前者完成拦截赋值操作**        **后者完成赋值的默认行为**

-   Reflect.set 会触发 Proxy.defineProperty 拦截操作 (只有 set 传入 receiver 的时候才会)

##  for of 调用的是对象的 iterator 接口

-   遍历 map        for (let [key,value] of map) {};

    -   for (let value of map) {}   // ['a',1]  ['b',2]    map 的数据输出

-   类数组对象转换为数组        Array.from(arrayLike)       **Array.from()**

    -   类数组的数据结构 string nodeList arguments

-   返回值是迭代器对象 Object.keys(obj)     Object.values(obj)      obj.entries()
...
-   babel 文件的配置方式根据场景改变
    ...
    文件相关配置
    .babelrc.json文件，具有不同的扩展名 ( .babelrc, .js, .cjs, .mjs)
    - 流程
    - parse -> transform -> generate	
    - parse (语法分析 生成AST)
    - transform (遍历一次AST进行增删改 递归修改AST数据)
    - generate (将AST打印成目标字符串 生成sourcemap)
...
-   npm i -D @babel/plugin-transform-arrow-functions
    ...
    -   scripts 里面
        ...
        -   箭头函数转换为普通函数
            ...
            -   "babel-arrow-function": "babel --plugins @babel/plugin-transform-arrow-functions src -d dist"   

            -   npm run babel-arrow-function

        -   对象中的 function 函数声明方法兼容 const obj = {function fn() {}} 转换为 const obj = {fn: function () {}}

            -   "babel-block-function": "babel --plugins @babel/plugin-transform-block-functions src -d dist"

        -   变量的声明变换为块级 

            -   "babel-block": "babel --plugins @babel/plugin-transform-block-scoping src -d dist"
    ...
    -   插件的模块化
        ...
        -   npm i -D @babel/plugin-transform-module-amd

        -   scripts 里面    // dist 目录是自创建的

            -   "build:amd": "babel src/test.js -d dist/test-amd.js --plugins @babel/plugin-transform-modules-amd"

            -   "build:commonjs": "babel src/test.js -d dist/test-commonjs.js --plugins @babel/plugin-transform-modules-commonjs "

            -   "build": "npm run build:amd && npm run build:commonjs"

    -   代码体积减小插件

        -   @babel/plugin-minify-..     // (.. 代表插件具体名称)

    -   babel presets   反序执行

    -   @babel/polyfill babel-runtime babel-preset-env

        -   @babel/polyfill 垫片 但是体积较大

        -   babel-runtime 不会对实例对象原型的属性方法污染 不污染内置对象

        -   babel-preset-env 集成工具 可以按需引入
...
-   npm i webpack webpack-cli 
    ...
    -   webpack test.js --mode development
    ...
    -   打包 js 的同时生产 html 页面
        ...
        -   webpack.config.js
            ...
            -   npm i -D extract-text-webpack-plugin ts-loader tsc typescript
            ...
            -   const HtmlWebpackPlugin = require("html-webpack-plugin")
                ...
                module.exports = {
                    ...
                    output: {
                        ...
                        path: path.join(__diename, "release"),
                        <!-- 资源共享的时候需要放置 publicPath 
                        publicPath: "http://.."
                        -->
                    },
                    plugins: [
    ...
    -   自写简单插件
        ...
        -   引入插件 
            ...
            -   const ConsoleOnBuildWebpackPlugin = require("./pluginPath")
    ...
    -   webpack 的开发和生产环境的配置
        ...
        -   npm i -D webpack webpack-cli html-webpack-plugin css-loader style-loader less less-loader file-loader clean-webpack-plugin extract-text-webpack-plugin webpack-merge
        ...
        -   webpack.base.config.js  // 提炼出来 dev prod 公共的基础部分     

            -   npm i -D webpack-merge  // 配置文件合并
    ...
    -   手写一个 vue 的 webpack 搭建项目

        -   npm init -y

        -   npm i -D  vue-loader webpack webpack-cli html-webpack-plugin webpack-dev-server babel-loader css-loader vue-style-loader @babel/core vue-template-compiler less-loader less
